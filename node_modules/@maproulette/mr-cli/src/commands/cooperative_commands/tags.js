...
const generateCooperativeWork = async (context, {changes, elementMaps, elementDataSetsByType, references}) => {
  ...
  context.spinner.start(`${context.filename}: ${0}/${allChanges.length} tasks`)
  ...
  for (let i = 0; i < allChanges.length; i++) {
    ...
    const dependentOperations = await operationsFor(currentChange)
    ...
    context.spinner.text = `${context.filename}: ${i + 1}/${allChanges.length} tasks`
  }
}
...
exports.handler = async function(argv) {
  ...
  const context = { out, spinner, osmChange: argv.osc, josmChange: argv.josm }
  try {
    for (let i = 0; i < argv.inputFiles.length; i++) {
      ...
      const parsed = parser.parse(changeData)
