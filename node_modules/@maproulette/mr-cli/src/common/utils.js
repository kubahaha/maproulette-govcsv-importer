  ...
  serializeSupportingNodes: function(node, supportingNodesById, serializer) {
    ...
    for (let i = 0; i < node.childNodes.length; i++) {
      ...
      switch (child.nodeName) {
        case 'nd':
          refId = `node/${child.attributes.getNamedItem('ref').nodeValue}`
          break
        case 'member':
          refId =
            `${child.attributes.getNamedItem('type').nodeValue}/${child.attributes.getNamedItem('ref').nodeValue}`
          break
      }

      if (refId) {
        const referenced = supportingNodesById.get(refId)
        if (referenced) {
