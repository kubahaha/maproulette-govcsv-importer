...
def check_files(name, prepare, download):
    ...
    if download:
        ...
        is_overpass = '' if os.path.isfile(f'data/{name}/query.overpass') else '[bold red] '
    else:
        is_osm = '' if os.path.isfile(f'data/{name}/data.osm') else '[bold red] '
        is_overpass = '' if os.path.isfile(f'data/{name}/query.overpass') else '[bold yellow] '
    ...
    workdir.add(f'{is_overpass}query.overpass')
    ...
    if is_osm:
        if download:
            ...
            should_fail = True
    if is_overpass:
        error_console.log(f'{is_overpass}Overpass query not saved in file `data/{name}/query.overpass`!')
        should_fail = True
    if is_out_coop:
